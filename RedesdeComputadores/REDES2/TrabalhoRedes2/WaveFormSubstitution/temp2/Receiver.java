	...
	public RTPpacket getPacket(int n) {
		...
		if(packts[n-1] == null)
		{
			...
			CrossCorrelation cc = new CrossCorrelation(
					bufferPreviousPacket,bufferPreviousPacket.length,		
					0,template.length,0,bufferPreviousPacket.length,CODE.BYTE_UNSIGNED);
			
			WaveformSubstitutionPatternAlgorithm wspa
			   = new WaveformSubstitutionPatternAlgorithm(bufferPreviousPacket,bufferPreviousPacket.length,0,template.length,0,cc);
